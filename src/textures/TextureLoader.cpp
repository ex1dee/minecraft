...
void TextureLoader::loadSprite(
	const nlohmann::basic_json<>& json, SpriteTexture& texture, bool unloadImage
) {
	...
	if (texture.useAtlas) {
		...
		std::shared_ptr<TextureAtlas> atlas = loadAtlas(path, imagesCount, true, TextureType::SPRITE, unloadImage);
		...
		texture.data = loadBasic(path, true, TextureType::SPRITE, unloadImage);
	}
}

std::shared_ptr<TextureAtlas> TextureLoader::loadAtlas(
	const std::string& path, const glm::vec2& imagesCount, bool flip, TextureType type, bool unloadImage
) {
	std::shared_ptr<TextureAtlas> texture = getTexture<TextureAtlas>(path);

	if (texture == nullptr) {
		textures.emplace(path, std::make_shared<TextureAtlas>(path, imagesCount, flip, type, unloadImage));
...
std::shared_ptr<BasicTexture> TextureLoader::loadBasic(
	const std::string& path, bool flip, TextureType type, bool unloadImage
) {
	std::shared_ptr<BasicTexture> texture = getTexture<BasicTexture>(path);

	if (texture == nullptr) {
		textures.emplace(path, std::make_shared<BasicTexture>(path, flip, type, unloadImage));
...
std::shared_ptr<CubeTexture> TextureLoader::loadCube(
	const std::string& directory, std::array<std::string, 6>& fileNames, bool flip, bool unloadImage
) {
	std::shared_ptr<CubeTexture> texture = getTexture<CubeTexture>(directory);

	if (texture == nullptr) {
		textures.emplace(directory, std::make_shared<CubeTexture>(directory, fileNames, flip, unloadImage));
